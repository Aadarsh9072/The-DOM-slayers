...
const data = [
  { name: 'Critical', value: 3, color: '#D1452C' },
  { name: 'Warning', value: 5, color: '#E8C547' },
  { name: 'Info', value: 4, color: '#4A90D9' },
];
...
export default function AlertSeverityDonut() {
  ...
  return (
    ...
    >
      <div className="mb-3">
        <h3 className="font-sans font-bold text-sm text-[var(--text-primary)]">Alert Severity</h3>
        <p className="text-xs text-[var(--text-secondary)] mt-0.5">Active alerts overview</p>
      </div>
        ...
        <div className="w-1/2 flex flex-col gap-2 pl-2">
          {data.map((d, index) => (
            ...
            >
              ...
              <span className="text-xs text-[var(--text-secondary)] flex-1 flex items-center">{d.name}<InfoTooltip term={d.name} /></span>
